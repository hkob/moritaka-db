...
機能: 楽器に関する CRUD を確認する
  ...
  シナリオ: 運用環境では楽器の追加ができない
    前提 運用環境である
    かつ 楽器一覧を表示する
      ならば 画面に "楽器の追加" と表示されていないこと

  シナリオ: 開発環境で新規楽器を作成する
    前提 タイトルにテスト用データを登録する
    かつ タイトル一覧を表示する
    もし "ほ" をクリックする
    かつ id が "ボーカル" である行の "楽器の追加" をクリックする
      ならば 画面に "楽器一覧" と表示されていること
  ...
  シナリオ: パラメータ設定が間違えていたときのエラー確認(楽器の編集)
    前提 楽器にテスト用データを登録する
    もし 楽器一覧を表示する
    かつ id が "編曲" である行の "編集" をクリックする
    かつ "並び順" に "" と入力する
    かつ "更新する" ボタンをクリックする
      ならば 画面に "並び順を入力してください。" と表示されていること
    もし 楽器一覧を表示する
    かつ "English" をクリックする
    かつ id が "Vocal" である行の "Edit" をクリックする
    かつ "Sort order" に "" と入力する
    かつ "Update Instrumental" ボタンをクリックする
      ならば 画面に "Sort order can't be blank" と表示されていること
